...
Starting from version 2.1, by setting the `[Proxy|Duck]Generator.CacheDirectory` property you can make the system cache the generated assembly, so next time your app starts and requires the proxy there won't be time consuming emitting operation. It's strongly recommended to have each app version their own cache directory e.g. `"tmp_folder\your_app\current_version\"`.
## Resources
